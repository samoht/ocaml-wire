(** Generate EverParse .3d files, run EverParse, and generate C/OCaml FFI stubs
    and benchmark loops from the schemas.

    Usage: gen_stubs.exe <schema_dir>

    Outputs:
    - <schema_dir>/*.3d -- EverParse 3D schema files
    - ep_stubs.c -- C stubs (via Wire.to_c_stubs) + timed loops
    - ep_stubs.ml -- OCaml externals (via Wire.to_ml_stubs) + loops
    - ep_dispatch.ml -- dispatch table mapping names to functions *)
...
let () =
  ...
  (* Step 4: Generate ep_dispatch.ml *)
  let oc = open_out "ep_dispatch.ml" in
  let pr fmt = Printf.fprintf oc fmt in
  ...
  Printf.printf "Generated ep_dispatch.ml\n"
