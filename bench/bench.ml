...
(* Read accessors *)
let get_minimal_value =
  Staged.unstage (Codec.get Demo.minimal_codec Demo.f_minimal_value)

let get_bf8_value = Staged.unstage (Codec.get Demo.bf8_codec Demo.f_bf8_value)
let get_bf16_id = Staged.unstage (Codec.get Demo.bf16_codec Demo.f_bf16_id)

let get_bool_active =
  Staged.unstage (Codec.get Demo.bool_fields_codec Demo.f_bool_active)

let get_bf32_pri = Staged.unstage (Codec.get Demo.bf32_codec Demo.f_bf32_pri)

let get_clcw_report =
  Staged.unstage (Codec.get Space.clcw_codec Space.cw_report)

let get_tcp_dst_port =
  Staged.unstage (Codec.get Net.tcp_codec Net.f_tcp_dst_port)
...
let set_bf8_value = Staged.unstage (Codec.set Demo.bf8_codec Demo.f_bf8_value)

let set_bool_active =
  Staged.unstage (Codec.set Demo.bool_fields_codec Demo.f_bool_active)

let set_clcw_report =
  Staged.unstage (Codec.set Space.clcw_codec Space.cw_report)

let set_tcp_dst_port =
  Staged.unstage (Codec.set Net.tcp_codec Net.f_tcp_dst_port)
...
let () =
  ...
  bench_flat "Minimal.value (uint8)" ~size:Demo.minimal_size
    ...
    ~buf:minimal_buf ~get_fn:(fun () ->
      ignore (get_minimal_value minimal_buf 0));
  ...
  bench_flat "Bitfield8.value (bf5 in bf_uint8)" ~size:Demo.bf8_size
    ...
    ~buf:bf8_buf ~get_fn:(fun () -> ignore (get_bf8_value bf8_buf 0));
  ...
  bench_flat "Bitfield16.id (bf11 in bf_uint16be)" ~size:Demo.bf16_size
    ...
    ~buf:bf16_buf ~get_fn:(fun () -> ignore (get_bf16_id bf16_buf 0));
  ...
  bench_flat "BoolFields.active (bool bf1 in bf_uint8)"
    ...
    ~check_fn:C_stubs.boolfields_check ~buf:bool_buf ~get_fn:(fun () ->
      ignore (get_bool_active bool_buf 0));
  ...
  bench_flat "Bitfield32.pri (bf8 in bf_uint32be)" ~size:Demo.bf32_size
    ...
    ~buf:bf32_buf ~get_fn:(fun () -> ignore (get_bf32_pri bf32_buf 0));
  ...
  bench_flat "CLCW.report (bf8 in bf32be)" ~size:Space.clcw_size
    ...
    ~get_fn:(fun () -> ignore (get_clcw_report clcw_buf 0));
  ...
  bench_flat "TCP.dst_port (uint16be)" ~size:Net.tcp_size
    ...
    ~get_fn:(fun () -> ignore (get_tcp_dst_port tcp_buf tcp_off));
  ...
  let tcp_syn_get =
    time_op n (fun () -> ignore (get_tcp_syn tcp_buf tcp_off))
  in
  let tcp_syn_alloc =
    alloc_words n (fun () -> ignore (get_tcp_syn tcp_buf tcp_off))
  in
  ...
  bench_flat "AllInts.u64be (uint64be, boxed)" ~size:Demo.all_ints_size
    ...
    ~buf:ints_buf ~get_fn:(fun () -> ignore (get_ints_u64be ints_buf 0));
  ...
  bench_flat "LargeMixed.timestamp (uint64be, 26B)" ~size:Demo.large_mixed_size
    ...
    ~buf:mixed_buf ~get_fn:(fun () -> ignore (get_mixed_timestamp mixed_buf 0));
  ...
  bench_flat "IPv4.src (uint32be, unboxed)" ~size:Net.ipv4_size
    ...
    ~get_fn:(fun () -> ignore (get_ip_src tcp_buf ip_off));
  ...
  let nested_dst_get =
    time_op n (fun () ->
        ...
        ignore (get_tcp_dst_port tcp_buf tcp))
  in
  ...
  let nested_syn_get =
    time_op n (fun () ->
        let ip = sub_eth_payload tcp_buf 0 in
        let tcp = sub_ip_payload tcp_buf ip in
        ignore (get_tcp_syn tcp_buf tcp))
  in
  ...
  (* Minimal.value *)
  let v = time_op n (fun () -> set_minimal_value minimal_buf 0 42) in
  let a = alloc_words n (fun () -> set_minimal_value minimal_buf 0 42) in
  write_row "Minimal.value (uint8)" v a;

  (* Bitfield8.value *)
  let v = time_op n (fun () -> set_bf8_value bf8_buf 0 19) in
  let a = alloc_words n (fun () -> set_bf8_value bf8_buf 0 19) in
  write_row "Bitfield8.value (bf5, read-mod-write)" v a;

  (* BoolFields.active *)
  let v = time_op n (fun () -> set_bool_active bool_buf 0 true) in
  let a = alloc_words n (fun () -> set_bool_active bool_buf 0 true) in
  write_row "BoolFields.active (bool bf1, rmw)" v a;

  (* CLCW.report: read-modify-write bitfield *)
  let v = time_op n (fun () -> set_clcw_report clcw_buf 0 42) in
  let a = alloc_words n (fun () -> set_clcw_report clcw_buf 0 42) in
  write_row "CLCW.report (bf8, read-mod-write)" v a;

  (* TCP.dst_port: direct uint16be write *)
  let v = time_op n (fun () -> set_tcp_dst_port tcp_buf tcp_off 8080) in
  let a = alloc_words n (fun () -> set_tcp_dst_port tcp_buf tcp_off 8080) in
  write_row "TCP.dst_port (uint16be)" v a;

  (* TCP.syn: read-modify-write bool bitfield *)
  let v = time_op n (fun () -> set_tcp_syn tcp_buf tcp_off true) in
  let a = alloc_words n (fun () -> set_tcp_syn tcp_buf tcp_off true) in
