...
let bf8_value = Staged.unstage (Codec.get Demo.bf8_codec Demo.f_bf8_value)
let bf16_id = Staged.unstage (Codec.get Demo.bf16_codec Demo.f_bf16_id)

let bool_active =
  Staged.unstage (Codec.get Demo.bool_fields_codec Demo.f_bool_active)

let bf32_pri = Staged.unstage (Codec.get Demo.bf32_codec Demo.f_bf32_pri)
let clcw_report = Staged.unstage (Codec.get Space.clcw_codec Space.cw_report)
let tcp_dst_port = Staged.unstage (Codec.get Net.tcp_codec Net.f_tcp_dst_port)
let tcp_syn = Staged.unstage (Codec.get Net.tcp_codec Net.f_tcp_syn)
...
let () =
  ...
  bench_flat "Minimal.value (uint8)" ~size:Demo.minimal_size
    ...
    ~buf:minimal_buf ~get_fn:(fun () -> ignore (minimal_value minimal_buf 0));
  ...
  bench_flat "Bitfield8.value (bf5 in bf_uint8)" ~size:Demo.bf8_size
    ...
    ~buf:bf8_buf ~get_fn:(fun () -> ignore (bf8_value bf8_buf 0));
  ...
  bench_flat "Bitfield16.id (bf11 in bf_uint16be)" ~size:Demo.bf16_size
    ...
    ~buf:bf16_buf ~get_fn:(fun () -> ignore (bf16_id bf16_buf 0));
  ...
  bench_flat "Bitfield32.pri (bf8 in bf_uint32be)" ~size:Demo.bf32_size
    ...
    ~buf:bf32_buf ~get_fn:(fun () -> ignore (bf32_pri bf32_buf 0));
  ...
  bench_flat "CLCW.report (bf8 in bf32be)" ~size:Space.clcw_size
    ...
    ~get_fn:(fun () -> ignore (clcw_report clcw_buf 0));
  ...
  bench_flat "TCP.dst_port (uint16be)" ~size:Net.tcp_size
    ...
    ~get_fn:(fun () -> ignore (tcp_dst_port tcp_buf tcp_off));
  ...
  let tcp_syn_get = time_op n (fun () -> ignore (tcp_syn tcp_buf tcp_off)) in
  let tcp_syn_alloc =
    alloc_words n (fun () -> ignore (tcp_syn tcp_buf tcp_off))
  in
  ...
  bench_flat "AllInts.u64be (uint64be, boxed)" ~size:Demo.all_ints_size
    ...
    ~buf:ints_buf ~get_fn:(fun () -> ignore (ints_u64be ints_buf 0));
  ...
  bench_flat "LargeMixed.timestamp (uint64be, 26B)" ~size:Demo.large_mixed_size
    ...
    ~buf:mixed_buf ~get_fn:(fun () -> ignore (mixed_timestamp mixed_buf 0));
  ...
  bench_flat "IPv4.src (uint32be, unboxed)" ~size:Net.ipv4_size
    ...
    ~get_fn:(fun () -> ignore (ip_src tcp_buf ip_off));
