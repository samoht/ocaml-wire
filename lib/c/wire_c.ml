...
let size_of_struct = Wire.size_of_struct
let ml_type_of = Wire.ml_type_of
...
let c_stub_check ppf (s : Wire.struct_) =
...
let to_c_stubs (structs : Wire.struct_ list) =
  ...
  List.iteri
    ...
    structs;
  Fmt.pf ppf "@\n/* Validation stubs */@\n";
  List.iter (fun (s : Wire.struct_) -> c_stub_check ppf s) structs;
...
let to_ml_stubs (structs : Wire.struct_ list) =
...
let to_ml_stub_name (s : Wire.struct_) =
  let name = Wire.struct_name s in
